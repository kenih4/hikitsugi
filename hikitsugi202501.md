...
- [ ] 6/9 読み値に変動がみられたCB12-4について　KLY流量計清掃
- [ ] 6/15 電子銃 Ik、G1、G2の変動が4月末と比較して大きくなっている 
- [ ] 6/16 BL同期レーザとFELが数psズレている件
- [ ] 6/16 CB14-2 Thy交換 ユニット切替　シード出力2割低下

-流行り： LB Grid変動、カソード電圧、GUN Thy
-次回マシン調整： LB　以前加速器調整時に7Vまで調整する提案があったが、直近1週間においてG1電圧の変動が収まっているためこのままの状態（6.8V)で様子見する
-不具合箇所：
　CB01-1 増加傾向にあったが今の所,自爆頻度は低下, scm_bta_1_2_stage_ccdモータードライバーの故障　夏対処
...
-MCH（μTCA管理モジュール）のログを取得し連絡するように指示を受けた (保存名:ホスト,日付)					
以下のページ(制御LAN）					
