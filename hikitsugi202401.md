...
- [ ] 5/24 CB02-2、BC3のラック(共にラック2)にてTEMP WARNINGによるアラームが本シフト中に複数回発生(CB02-2は5/23 1直中にも発生)
- [x] 5/24 パラメータ変更時、安定になるようにメインライナック+1度
- [x] 5/24 シード 9.83keV 60uJ 強度低下　エネルギーFB、異常に振る
...
- [ ] 5/19 アボート　Bstポンプ停止 閾値緩めた、ジャンパーしてRF電源室 Bst パネルメーターの背面端子にデータロガーを接続
- [ ] 5/20 マシン調整
・「空洞のHOMの観測と不安定性の確認」
...
- [x] 5/20 ID09のGAPが5/20 15時頃より、GFO状態にも関わらず断続的に49.984mmとなる。サブエンコーダーへ切替
- [x] 5/24 BL11XUにて漏水を発見、止水して様子見
