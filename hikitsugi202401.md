...
- [x] 年度末停止　パルスQ
- [x] 年度末停止kicker sub機(MS-0)で運転開始。年度末停止期間以前はmain機（MS-2）。
...
- [x] GW作業　SACLA制御室にあるインターロックPLC用UPSの交換作業を実施しましたが、バッテリー温度異常のエラーが発生したため夏期停止期間までバイパスモードで運転
- [ ] GW作業　B_BTAの電流モニター運転インターロックでふらつく件、端子を全て交換
- [x] 5/8 LB モジュレータ電源のサイラトロン・グリッド電圧がスパイクし、加速管の高周波位相が変動した。これにより一時的に FEL の強度が著しく低下したがすぐに復旧した。
- [x] 5/8 ML_GUNを振ってもcharge感度がない件で、入射部軸出し。
...
- [x] 4/10 Bst L2漏水 様子見
- [x] 4/13 TopUp入射遅延あり
- [x] 4/17 ビームアボート RF C-st Cav3反射異常でRF down
